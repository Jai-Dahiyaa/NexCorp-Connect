...
import { loginOTPService, loginOTPRefreshToken } from "../../services/auth/loginOTP.service.js";
...
const loginOTPController = catchAsync(async (req, res) => {
  ...
  const generateLoginOTPToken = utilsToken.accessTokenGenerate(payload);
  ...
  res.status(200).json({status: true, message: "Login OTP send your email Successfully" });
});
...
const loginOTPuserVerify = catchAsync(async (req, res) => {
  ...
  const refreshTokenInsert = await loginOTPRefreshToken(payload.id, refreshToken, tokenExpiresAt, userAgent, ipAddress);
  ...
  res.status(200).json({status: true, message: "User Login SuccessFully", users: users.result });
});
