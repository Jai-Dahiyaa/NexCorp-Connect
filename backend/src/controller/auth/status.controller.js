...
const statusChangeController = catchAsync(async (req, res) => {
  ...
  if (status === "true") {
    ...
    res.status(200).json({status: true, message: "User Status SuccessFull Change", user: statusVerifyEnter.userStatus });
  }

  if (status === "false") {
    const statusVerifyEnter = await statusService.userStatusFalse(email);
    res.status(200).json({status: true, message: "User Status SuccessFull Change", users: statusVerifyEnter.userStatus });
  }
});
